    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.CreateAsync(memberUser, model.Password);
                //if successful..
                if (result.Succeeded)
                {

                    //.. then add user to member role
                    await UserManager.AddToRoleAsync(memberUser.Id, "Member");

                    await SignInManager.SignInAsync(memberUser, isPersistent:false, rememberBrowser:false);
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var member = new Member { UserName = model.Email, Email = model.Email };
                var result = await UserManager.CreateAsync(member);
                if (result.Succeeded)
                {
                    result = await UserManager.AddLoginAsync(member.Id, info.Login);
                    if (result.Succeeded)
                    {
